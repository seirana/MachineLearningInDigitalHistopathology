...
for slide_th in slides.item():  
    if slides.item().get(slide_th)['slide_ID'] == '04A-C_MaLTT_Ctrl72h_Casp3_MaLTT_Ctrl72h_Casp3_04A-C - 2015-07-04 22.39.34':
        ...
        for tissue_th in slides.item().get(slide_th)['tissues']:
            for h in range(slides.item().get(slide_th)['tissues'][tissue_th]['up']+1,slides.item().get(slide_th)['tissues'][tissue_th]['down']):
                for w in range(slides.item().get(slide_th)['tissues'][tissue_th]['left']+1,slides.item().get(slide_th)['tissues'][tissue_th]['right']):
